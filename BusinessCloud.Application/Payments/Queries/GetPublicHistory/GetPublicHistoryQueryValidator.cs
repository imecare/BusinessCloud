...
public class GetPublicHistoryQueryValidator : AbstractValidator<GetPublicHistoryQuery>
{
    public GetPublicHistoryQueryValidator()
    {
        RuleFor(x => x.Phone)
            ...
            .Matches(@"^\+?\d{7,15}$").WithMessage("El tel?fono debe tener entre 7 y 15 d?gitos.");
        ...
        RuleFor(x => x.CompanyCode)
            .NotEmpty().WithMessage("El c?digo de empresa es obligatorio.")
            .MaximumLength(50).WithMessage("El c?digo de empresa no puede superar 50 caracteres.");
    }
}