...
public class ProcessDeliveryBatchHandler(IBazaresDbContext context) : IRequestHandler<ProcessDeliveryBatchCommand, ProcessDeliveryBatchResultDto>
{
 ...
 {
  var ids = request.ClosureEventIds.Distinct().ToList(); var affected = 0; int? targetId = null;
  await context.ExecuteInTransactionAsync(async transactionCt =>
  {
   ...
   else if (totals.Count > 0)
   {
    ...
    foreach (var total in totals) total.BzaClosureEventId = target.Id; foreach (var sale in sales) sale.BzaClosureEventId = target.Id; if (target.Status == BzaClosureEventStatus.Validated) target.Status = BzaClosureEventStatus.PendingPayment; targetId = target.Id;
   }
   foreach (var closure in closures) closure.InDeliveryProcess = true; await context.SaveChangesAsync(transactionCt);
  }, ct);
